...
from ENKA_CARD.character_card.prop_reference import ELEMENT_REFERENCE, RELIQUARY_STATS
...
# Update font loading
fonts_dir = os.path.join(os.path.dirname(__file__), 'attributes', 'Fonts')
assets_dir = os.path.join(os.path.dirname(__file__), 'attributes', 'Assets')

def get_font(font: Literal["normal"], size: int) -> ImageFont.FreeTypeFont:
    """Helper method to get a font."""
    return {
        "normal": ImageFont.truetype(os.path.join(fonts_dir, "JA-JP.TTF"), size),
        # Insert other fonts you'd like to use here, if any
    }.get(font, ImageFont.truetype(os.path.join(fonts_dir, "JA-JP.TTF"), size))


def fade_character_art(im: Image) -> Image:
    # Load mask from attributes
    mask = Image.open(os.path.join(assets_dir, "enka_character_mask.png")).convert("L")
...
def fade_asset_icon(im: Image, _type: Literal["artifact"]) -> Image:
    mask_fp = {
        "artifact": os.path.join(assets_dir, "artifact_mask.png"),
